...
export function createSelection(type: string) {
	...
	var b: dynamicgraph.Selection = dgraph.createSelection(type) // IS IT OK?? (dgraph)
...
export function updateList(type: string, name: string) {
	var selections: dynamicgraph.Selection[] = dgraph.getSelections(type)
	...
	nodeGs.append('svg:image')
		...
		.on('click', function (d: dynamicgraph.Selection, i: number) {
	...
	nodeGs.append('svg:image')
		...
		.attr('x', 130 + (RECT_SIZE + GAP_ICONS) * i++)
		.on('click', function (d: dynamicgraph.Selection, i: any) {
	...
	nodeGs.append('svg:image')
		...
		.on('click', function (d: dynamicgraph.Selection, i: number) {
			if (i > 0)
				messenger.swapPriority(d, <dynamicgraph.Selection>d3.selectAll('.selectionDiv_' + d.acceptedType).data()[i - 1]); // CAST TO SELECTION??
		})
...
export function saveSearchResultAsSelection(type: string) {
	var s: dynamicgraph.Selection = messenger.createSelection(type, searchMessage.searchTerm);
	var selectionIdCompound: utils.IDCompound = new utils.IDCompound();
